      ...
      <TouchableOpacity style={styles.signOutBtn} onPress={() =>
        Alert.alert(
          ...
          [
            ...
            { text: lang === 'tr' ? 'Çıkış yap' : 'Sign out', style: 'destructive', onPress: signOut }
          ]
        )
      }>
        <Text style={styles.signOutText}>{lang === 'tr' ? 'Çıkış yap' : 'Sign out'}</Text>
      </TouchableOpacity>

      <Text style={styles.disclaimer}>
        {lang === 'tr'
          ? 'MindBridge bir terapistin yerini almaz.\nKriz: 182 (TR) · 988 (US)'
          : 'MindBridge is not a substitute for professional care.\nCrisis: 182 (TR) · 988 (US)'}
      </Text>
    </ScrollView>
  )
}
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 11, fontWeight: '600', color: colors.gray400, textTransform: 'uppercase', letterSpacing: 0.5, padding: spacing.md, paddingBottom: 6 },
  settingRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: spacing.md, borderTopWidth: 1, borderTopColor: colors.gray100 },
  settingLabel: { fontSize: 14, color: colors.gray900 },
  settingValue: { fontSize: 13, color: colors.gray400 },
  signOutBtn: { backgroundColor: colors.white, borderRadius: radius.md, borderWidth: 1, borderColor: '#F09595', padding: spacing.md, alignItems: 'center', marginBottom: spacing.md },
  signOutText: { color: colors.red400, fontSize: 15, fontWeight: '600' },
  disclaimer: { textAlign: 'center', fontSize: 11, color: colors.gray400, lineHeight: 18 }
})
